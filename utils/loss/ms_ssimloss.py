...
if __name__ == '__main__':
    ...
    pred = torch.randn((6, 21, 256, 256))  # (batch_size, num_classes, height, width)
    target = torch.randint(0, 21, (6, 256, 256))  # (batch_size, height, width)

    ssim_loss = SSIMLoss()
    ms_ssim_loss = MS_SSIMLoss()

    print(f'SSIM Loss: {ssim_loss(pred, target)}')
    print(f'MS-SSIM Loss: {ms_ssim_loss(pred, target)}')
