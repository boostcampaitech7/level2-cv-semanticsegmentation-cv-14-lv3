...
def parse_args():
    parser = argparse.ArgumentParser(description='Test Image Visualization')
    parser.add_argument('--test_data_dir', type=str, default='/data/ephemeral/home/data/test/DCM',
                        help='Test image가 있는 디렉토리 경로')
    parser.add_argument('--csv_path', type=str, default= 'u++_2048.csv')
...
def main() :
    ...
    id_folders = sorted([d for d in os.listdir(args.test_data_dir) if os.path.isdir(os.path.join(args.test_data_dir, d))])
    ...
    visualize = st.sidebar.checkbox("Show Annotations (Masks)", value=args.visualize)

    # 선택한 ID 폴더에서 이미지 파일 선택
    if selected_id:
        image_dir = os.path.join(args.test_data_dir, selected_id)
